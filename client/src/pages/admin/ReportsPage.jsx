...
const ReportsPage = () => {
	...
	const applyFilters = () => { const today = new Date(); const end = endDate || localDateKey(today); let start = startDate || end; if (period === 'weekly') { const date = new Date(`${end}T00:00:00`); date.setDate(date.getDate() - 6); start = localDateKey(date); } if (period === 'monthly') { const date = new Date(`${end}T00:00:00`); date.setDate(1); start = localDateKey(date); } void loadReport({ startDate: start, endDate: end, ...(department ? { department } : {}) }); };
