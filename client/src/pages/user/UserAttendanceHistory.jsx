...
const recordDateKey = (record) => record.attendanceDate || (record.date ? localDateKey(new Date(record.date)) : '');

const formatRecordDate = (record, options = { month: 'short', day: 'numeric', year: 'numeric' }) => {
	const key = recordDateKey(record);
	return key ? new Date(`${key}T00:00:00`).toLocaleDateString('en-US', options) : 'Unknown date';
};
...
const UserAttendanceHistory = () => {
	...
	useEffect(() => {
		if (!user?._id) return;
		attendanceAPI.history({ limit: 1000 })
	...
	const filteredRecords = useMemo(() => records.filter((record) => {
		...
		return (!query || haystack.includes(query.toLowerCase())) && (!date || recordDateKey(record) === date) && (department === 'all' || person?.department === department) && (location === 'all' || record.locationAddress === location);
	}), [records, user, query, date, department, location]);

	const stats = useMemo(() => {
		const monthPrefix = localDateKey(new Date()).slice(0, 7);
		const rateRecords = date
			? records.filter((record) => recordDateKey(record) === date)
			: records.filter((record) => recordDateKey(record).slice(0, 7) === monthPrefix);
		const present = rateRecords.filter((record) => record.status === 'Present').length;
		const late = rateRecords.filter((record) => record.status === 'Late').length;
		const absent = rateRecords.filter((record) => record.status === 'Absent').length;
		const total = rateRecords.length;
		const expectedDays = date ? 1 : new Date().getDate();
		return { present, late, absent, total, rate: expectedDays ? Math.round(((present + late) / expectedDays) * 100) : 0 };
	}, [records, date]);
	...
	const weeklyData = useMemo(() => Array.from({ length: 7 }, (_, index) => {
		...
		const dayRecords = filteredRecords.filter((record) => recordDateKey(record) === key);
	...
	const kpis = [
		...
		{ label: 'Attendance rate', value: `${stats.rate}%`, note: 'Present + late / elapsed days', icon: ShieldCheck, color: 'text-cyan-200', glow: 'from-cyan-300/20' },
	];
